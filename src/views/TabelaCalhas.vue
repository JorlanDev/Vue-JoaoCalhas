<template>
  ...
  <tbody>
    <tr v-for="preco in precocalhas" :key="preco.id" >
      <td >{{preco.Pano}}</td>
      <td>{{preco.Material}}</td>
      <td>{{preco.Pintura}}</td>
      <td>{{preco.Montagem}}</td>
      <td>{{preco.Preço.toLocaleString('pt-BR',{style:'currency', currency:'BRL'})}}</td>
      <td><button :value="preco.id" @click="ValueButtonCal" type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
            Editar
          </button>
      </td>
    </tr>
...
export default {
      ...
      created(){
        ...
        //Calhas
        axios.get("https://www.jorlan.shop/dados/calhas",).then(res => {
            console.log(res)
    ...
    methods:{
      ValueButtonCal: function(event){  // Nesta função estou pegando os valores que coloquei dentro dos botoes//
        let elementoClicado = (event.target);
        if (elementoClicado.value >= 1){
          this.ValorInputPreco = this.precocalhas.find(x => x.id == elementoClicado.value).Preço
      ...
      MaskMoeda: function(e){ //esta mascar coloca moeda com ponto na casa dos mil.
            ...
            this.ValorInputCalc = parseFloat(this.ValorInputPrecoIntl.replace(',','.'))//aqui estou dizendo que quando a mascara colocar uma "," ele ser transformado em um ponto.    w
        },
    ...
    Update: function(){
      axios.put("https://www.jorlan.shop:80/dados/calhas" + this.IdValue + "/" + this.ValorInputCalc,{
